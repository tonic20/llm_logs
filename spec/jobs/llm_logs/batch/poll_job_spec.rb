...
RSpec.describe LlmLogs::Batch::PollJob do
  it "reconciles every unreconciled batch that has an openai_batch_id" do
    b1 = LlmLogs::Batch.create!(purpose: "chat_summary", model: "m", status: "submitted", openai_batch_id: "b1")
    LlmLogs::Batch.create!(purpose: "chat_summary", model: "m", status: "reconciled", openai_batch_id: "b2")
    LlmLogs::Batch.create!(purpose: "chat_summary", model: "m", status: "pending", openai_batch_id: nil)
  ...
  it "recovers stale placeholder claims by reverting requests to pending and dropping the batch" do
    stale = LlmLogs::Batch.create!(purpose: "chat_summary", model: "m", status: "pending",
                                   openai_batch_id: nil, created_at: 1.hour.ago)
  ...
  it "does not recover a fresh placeholder claim" do
    fresh = LlmLogs::Batch.create!(purpose: "chat_summary", model: "m", status: "pending", openai_batch_id: nil)
    LlmLogs::Batch::PollJob.new.perform
